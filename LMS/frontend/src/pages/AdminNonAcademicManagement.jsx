...
export default function AdminNonAcademicManagement({ readOnly = false }) {
    ...
    const fetchStaff = async () => {
        ...
        try {
            ...
            const url = searchTerm
                ? `http://localhost:8080/admin/non-academic/search?term=${searchTerm}`
                : `http://localhost:8080/admin/non-academic/all`;
    ...
    const openAddModal = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch("http://localhost:8080/admin/non-academic/generate-id", {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch("http://localhost:8080/admin/non-academic/create", {
                method: "POST",
                headers: {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://localhost:8080/admin/non-academic/update/${editForm.nonAcademicId}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://localhost:8080/admin/non-academic/delete/${itemToDelete.nonAcademicId}?reason=${encodeURIComponent(deletionReason)}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
